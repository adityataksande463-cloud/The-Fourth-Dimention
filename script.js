 ...
 * script.js – Pure Imagination Edition
 * ==================================================================
 * Features:
 * - 30 techniques with full details (Sanskrit, steps, benefits)
 * - Breathing visualizer with 4-4-6 timing
 * - Gargantua black hole canvas animation
 * - Timer with start/pause/reset and practice recording
 * - My Plan cart with promo codes
 * - Profile with stats and recent activity
 * - Dark/light theme toggle with localStorage
 * - Fully responsive, smooth navigation
 * ==================================================================
 */
...
(function() {
    ...
    // ==============================================================
    // 1. TECHNIQUES DATABASE – 30 Portals
    // ==============================================================
    const techniques = {
        1: {
            id: 1,
    ...
    const AppState = (function() {
        const STORAGE_KEY = 'vigyan_bhairav';
        ...
        const defaultState = {
            ...
            user: {
                ...
                totalMinutes: 6304,
                streak: 28,
                history: [],                // { date, techniqueId, minutes, reflection? }
    ...
    const DOM = {
        ...
        profilePractices: document.getElementById('profilePractices'),
    ...
    // ==============================================================
    // 4. UI RENDERING – Painting the cosmic interface
    // ==============================================================
    const UI = {
        applyTheme: (theme) => {
        ...
        createTechCard: (tech) => {
            ...
            card.innerHTML = `
                <div class="tech-header">
                    ...
                    <button class="favorite-btn ${isFav ? 'fas' : 'far'} fa-heart" data-id="${tech.id}"></button>
                </div>
                <h4>${tech.name}</h4>
        ...
        renderFeatured: () => {
            ...
            const featuredIds = [1, 2, 3, 4]; // first four techniques
        ...
        showTechniqueDetail: (id) => {
            ...
            if (tech.breathTiming && DOM.breathVisual) {
                ...
                if (DOM.breathVisual) DOM.breathVisual.classList.add('hidden');
                BreathVisual.stop();
            }
        ...
        renderPlan: () => {
            if (!DOM.planItems) return;
            const state = AppState.get();
            const plan = state.user.plan;
            DOM.planItems.innerHTML = '';
    ...
    // ==============================================================
    // 5. BREATHING VISUALIZER – Animated circle of life
    // ==============================================================
    ...
    const Views = {
        show: (viewName) => {
            const views = ['home', 'detail', 'techniques', 'timer', 'plan', 'profile'];
            views.forEach(v => {
                const el = DOM[`${v}View`];
            ...
            if (viewName === 'techniques') UI.renderAllTechniques();
                ...
                if (DOM.profileStreak) DOM.profileStreak.textContent = u.streak;
                if (DOM.profilePractices) DOM.profilePractices.textContent = u.totalPractices;
                if (DOM.recentActivity) {
                    DOM.recentActivity.innerHTML = '';
                    u.history.slice(0, 3).forEach(h => {
                        const tech = techniques[h.techniqueId];
                        const el = document.createElement('div');
                        el.className = 'plan-item';
                        el.innerHTML = `<span>${tech ? tech.name : 'Unknown'}</span><span>${new Date(h.date).toLocaleDateString()}</span>`;
                        DOM.recentActivity.appendChild(el);
                    });
                }
            }
        }
    ...
    function initEventListeners() {
        ...
        DOM.backBtn.addEventListener('click', () => Views.show('techniques'));
        ...
        // Cart icon (navigates to plan)
        document.querySelector('.cart-icon')?.addEventListener('click', () => Views.show('plan'));
    }

    // ==============================================================
    // 8. GARGANTUA BLACK HOLE CANVAS – Pure animation
    // ==============================================================
    (function initBlackHole() {
        ...
        function drawBlackHole() {
            ...
            outerGradient.addColorStop(0, 'rgba(150, 100, 255, 0.5)');
            ...
            // 4. Accretion disk texture – swirling lines
            ctx.save();
            ctx.translate(centerX, centerY);
            ctx.rotate(time * 0.01); // slow rotation
            ...
            // 5. Einstein ring – thin bright ring
            ctx.beginPath();
            ...
            // 6. Gravitational lensing effect – stretched stars
            for (let s = 0; s < 30; s++) {
    ...
    function init() {
        ...
        console.log('🌌 Vigyan Bhairav – 30 portals open.');
    }
